module ObjectStorageNg
  class ContainersController < ::AjaxController
    ...
    def update_access_control
      container_name = params.require(:id)
      attrs = params.require(:metadata).permit('x-container-read', 'x-container-write')
      
      metadata = {
        'x-container-read': (attrs['x-container-read'] || "").delete("\r\n"),
        'x-container-write': (attrs['x-container-write'] || "").delete("\r\n")
      }
      # byebug
      code = services.object_storage_ng.update_container_metadata(container_name,metadata)
      if code.to_i >= 400
