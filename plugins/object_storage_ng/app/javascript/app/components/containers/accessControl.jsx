...
const ContainerAccessControl = ({}) => {
  ...
  const checkAcls = React.useCallback(
    ({ read, write }) => {
      ...
      apiClient
        .get(`containers/${name}/check-acls`, { read, write })
        .then((result) => setAclCheckResult(result))
        .catch((error) => {
          setError(error.message)
        })
  ...
  const submit = React.useCallback(
    (values) => {
      ...
      let newValues = {
        "x-container-read": values.read,
        "x-container-write": values.write,
      }

      return (
        apiClient
          .put(`containers/${name}/access-control`, { metadata: newValues })
          // close modal window
          .then(close)
          .catch((error) => {
